        ...
        if (selectedDate) {
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    // Populate new times
                    data.start_times.forEach(startTime => {
                        data.end_times.forEach(endTime => {
                            const option = document.createElement('div');
                            option.innerHTML = `<label><input type="radio" name="time" value="${startTime}-${endTime}" /> ${startTime} - ${endTime}</label><br/>`;
